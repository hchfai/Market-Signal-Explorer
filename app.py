"""Market Signal Explorer — Streamlit dashboard.

Educational decision-support tool for stocks and crypto. Not financial
advice, not a broker, and not a guarantee of anything. See README.md.
"""
...
with st.sidebar:
    ...
    if asset_type == "Stock":
        ...
        symbol = st.text_input("Coin symbol or CoinGecko id (e.g. BTC, ETH, bitcoin)", value="BTC")
    period_days = st.slider("History (days)", 90, 730, 365)
    starting_capital = st.number_input("Starting capital (AUD)", min_value=10.0, value=500.0, step=10.0)
...
if not TICKER_RE.match(symbol):
    st.error("That doesn't look like a valid ticker/symbol. Use letters, numbers, '.' or '-' only.")
    st.stop()
...
with st.spinner("Fetching price data..."):
    if asset_type == "Stock":
        ...
        df = get_crypto_history(coin_id, days=period_days, api_key=cfg.get("COINGECKO_API_KEY"))
        label = symbol.upper()
        currency_note = (
...
with price_col:
    ...
    fig.add_trace(go.Candlestick(
        x=df.index, open=df["Open"], high=df["High"], low=df["Low"], close=df["Close"], name=label
    ))
...
if headlines:
    ...
    st.caption("Add a free NEWSAPI_KEY to your .env to include news sentiment (see README.md).")
